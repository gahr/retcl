...
`*{package}* create _?objectName?_ _?-noconnect?_ _?host?_ _?port?_`

`set r [*{package}* new _?-noconnect?_ _?host?_ _?port?_]`
...
The {package} module is an event-driven, object-oriented, Redis client library
for the Tcl programming language. The library exposes a single `{package}`
...
Other than a few book-keeping methods, `{package}` instances transparently
...
Redis server by concatenating all arguments, effectively making `{package}`
instances completely decoupled from any version of Redis.  This has several
advantages:

* A `{package}` instance does not need to know about the semantics of a
...
Create an instance *r* of `{package}`. By default, the client automatically
...
straightforwards. Any methods not directly understood by the `{package}` class
...
By default, `{package}` objects operate in asynchronous mode: they return
